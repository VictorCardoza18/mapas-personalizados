...
class Metro : AppCompatActivity(),
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        try {
            ...
            mapView = MapView(
                context = this,
                mapResourceId = R.drawable.mapa_metro // Usa la imagen de la cafe de la ESCOM
            )
            ...
            mapView.post {
                ...
                mapView.setCurrentMap(MapMatrixProvider.MAP_METRO, R.drawable.mapa_metro)

                // Configurar el playerManager
                mapView.playerManager.apply {
                    setCurrentMap(MapMatrixProvider.MAP_METRO)
                    localPlayerId = playerName
                    updateLocalPlayerPosition(gameState.playerPosition)
                }

                Log.d(TAG, "Set map to: " + MapMatrixProvider.MAP_METRO)

                // Importante: Enviar un update inmediato para que otros jugadores sepan dónde estamos
                if (gameState.isConnected) {
                    serverConnectionManager.sendUpdateMessage(playerName, gameState.playerPosition, MapMatrixProvider.MAP_METRO)
                }
            }
        } catch (e: Exception) {
    ...
    private fun checkPositionForMapChange(position: Pair<Int, Int>) {
        ...
        when {
            ...
            position.first == 30 && position.second == 14 -> {
                canChangeMap = true
    ...
    private fun updatePlayerPosition(position: Pair<Int, Int>) {
        runOnUiThread {
            ...
            if (gameState.isConnected) {
                serverConnectionManager.sendUpdateMessage(playerName, position, "mapa_metro")
            }
    ...
    override fun onPositionReceived(device: BluetoothDevice, x: Int, y: Int) {
        runOnUiThread {
            ...
            mapView.updateRemotePlayerPosition(deviceName, Pair(x, y), MapMatrixProvider.MAP_METRO)
            mapView.invalidate()
        }
    }
                    ...
                    "positions" -> {
                        ...
                        players.keys().forEach { playerId ->
                            if (playerId != playerName) {
                                ...
                                if (map == MapMatrixProvider.MAP_METRO) {
